...
def phillips_elev_spectrum(Scale,Power,NLCoupling,powExponent):
    ...
    print 'Calculating the Phillips power spectrum...'
    ...
    print 'dk = ',dk,' meters^{-1}'
    print 'Nyquist Wavenumber = ',np.double(N/2)*dk,' meters^{-1}'
    ...
    k_gamma = np.min(np.sqrt(rho*g/gammaT),k_max)	# Upper wavenumber cutoff for the capillary wave region
    ...
    ### and bound waves
    NLCoupling.Nbound = np.shape(sourceIndex)[0]
    NLCoupling.bound = NLCoupling.free1 = NLCoupling.free2 = np.zeros(np.shape(sourceIndex)[0])

    print "There are ",NLCoupling.Nbound," bound-free pairs"
    ...
    powExponent = np.double(powExponent)   #-- power law spectra are proportional to k^(-powExponent)
